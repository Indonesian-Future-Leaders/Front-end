...
const button = cva("button", {
  variants: {
    intent: {
      ...
      navigation: "text-light-1 scale-100 hover:scale-110 !text-base lg:!text-lg hover:text-light-2",
      ...
      google: "bg-dark-1 text-light-1 rounded-full hover:bg-gray-900 flex items-center gap-2 mx-auto",
      details: "text-primary-1 hover:text-primary-2 underline inline-block",
      underline:
...
const link = cva("link", {
  variants: {
    intent: {
      ...
      dropdown: "bg-light-1 text-primary-1 w-full hover:bg-light-2 !text-sm py-1 text-center px-2",
...
export const Button = ({ className, intent, size, type, ariaLabel, ...props }) => (
  <button
...
export const Links = ({ className, intent, ariaLabel, ...props }) => (
  <HashLink
    aria-label={`link${ariaLabel ? "-" + ariaLabel : ""}`}
    smooth
    className={`${link({
      intent,
      className,
